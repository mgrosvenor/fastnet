...
for PTYPE in PTYPES:
    ...
    packet_count = i 
    outdir  = "experiments/layout_hbo"
    test_id = "%02i-%010i" % (typ,packet_count)
    args.append( "%s/%s.stats %4.2fMB-%02i" % (outdir, test_id, packet_count * 2048 / 1024.0 / 1024.0, typ )) 
    typ += 1

cmd = "./plot_fast_net.py RD %s layout-hbo-rd.pdf" % (" ".join(args) )
print cmd
run_proc(cmd,False)

cmd = "./plot_fast_net.py WR %s layout-hbo-wr.pdf" % (" ".join(args) )
print cmd
run_proc(cmd,True)

cmd = "./plot_fast_net.py APRD %s layout-hbo-aprd.pdf" % (" ".join(args) )
print cmd
run_proc(cmd,False)

cmd = "./plot_fast_net.py APWR %s layout-hbo-apwr.pdf" % (" ".join(args) )
print cmd
run_proc(cmd,True)
